...
server.tool("get_auth_token", "Get OAuth 2.0 access token from BOG iPay.", {},
  async () => ({ content: [{ type: "text", text: await handleGetAuthToken() }] }));

server.tool("create_order", "Create a new payment order.", createOrderSchema.shape,
  async (params) => ({ content: [{ type: "text", text: await handleCreateOrder(params) }] }));

server.tool("get_order_status", "Get order status by ID.", getOrderStatusSchema.shape,
  async (params) => ({ content: [{ type: "text", text: await handleGetOrderStatus(params) }] }));

server.tool("refund_order", "Refund an order (full or partial).", refundOrderSchema.shape,
  async (params) => ({ content: [{ type: "text", text: await handleRefundOrder(params) }] }));

server.tool("create_recurring", "Set up a recurring payment.", createRecurringSchema.shape,
...
server.tool("preauth_order", "Pre-authorize (hold) an amount on a card.", preauthOrderSchema.shape,
  async (params) => ({ content: [{ type: "text", text: await handlePreauthOrder(params) }] }));

server.tool("complete_preauth", "Complete/capture a pre-authorized order.", completePreauthSchema.shape,
  async (params) => ({ content: [{ type: "text", text: await handleCompletePreauth(params) }] }));
